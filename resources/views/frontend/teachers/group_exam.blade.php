    ...
    <div class="container" style="position: relative; z-index: 1;">
        ...
        <div class="marks-table-wrapper">
            <div style="padding: 14px 16px; background: var(--primary);">
                <span style="color: white; font-size: 17.2px; font-weight: 700;"><i class="fa fa-list" style="color: var(--accent);"></i> Group Exam Dates</span>
            </div>
                        ...
                        @forelse ($data as $group)
                            <tr>
                                <td style="text-align:left; padding-left:16px;">
                                    <div style="font-weight: 700; color: var(--primary); font-size: 16.2px;">{{ $group->group->name }}</div>
                                    @if($group->group->program ?? null)
                                        <div style="font-size: 14.2px; color: var(--light-text);">{{ $group->group->program->title }}</div>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="{{ url('examDate/' . $group->group_id) }}" class="action-btn-xs evaluate" title="Edit Exam Dates" style="width:auto; padding: 0 12px; font-size: 15.2px; gap:5px; display:inline-flex;">
                            ...
                            <tr>
                                <td colspan="5" class="text-center" style="padding: 60px 20px; color: var(--light-text);">
                                    <div style="width: 100px; height: 100px; background: rgba(49,130,206,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px auto;">
                                        <i class="fa fa-calendar-times-o" style="font-size: 43.2px; color: #3182ce;"></i>
                                    </div>
                                    <h4 style="color: var(--primary); font-weight: 700; margin-bottom: 5px;">No Exams Scheduled</h4>
                                    <p style="font-size: 16.2px; margin-bottom: 15px;">There are currently no active groups with scheduled exams.</p>
