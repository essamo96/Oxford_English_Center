...
<style>
    ...
    .step-dot { position:relative; z-index:3; width:30px; height:30px; border-radius:50%; background:white; border:3px solid #e2e8f0; display:flex; align-items:center; justify-content:center; font-size: 13.2px; font-weight:700; color:#a0aec0; transition:all .3s; flex-shrink:0; }
    .step-dot.done { background:var(--accent); border-color:var(--accent); color:var(--primary); box-shadow:0 0 10px rgba(245,197,24,.35); }
    .step-dot .dot-label { position:absolute; top:36px; left:50%; transform:translateX(-50%); white-space:nowrap; font-size: 13.2px; color:#718096; font-weight:600; }
    .step-dot.done .dot-label { color:var(--primary); }
    .pct-badge { font-size: 29.2px; font-weight:800; color:var(--accent); line-height:1; }
</style>
@endsection
    ...
    <div class="container" style="position: relative; z-index: 1;">
        ...
        @forelse ($info as $group)
            ...
            <div class="progress-card animate-up">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-10">
                    <div>
                        <div style="font-size: 19.2px; font-weight: 700; color: var(--primary);">{{ $group->name }}</div>
                        @if($group->program ?? null)
                            <div style="font-size: 15.2px; color: var(--light-text);">{{ $group->program->title }}</div>
                        @endif
                    </div>
                    <div class="text-right">
                        <div class="pct-badge">{{ $pct }}%</div>
                        <div style="font-size: 14.2px; color: var(--light-text); margin-top:2px;">Completed</div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center" style="margin-top: 8px;">
                    <div class="d-flex gap-15" style="font-size: 15.2px; color: var(--light-text);">
                    ...
                    <a href="{{ route('teacher.showGroueStudents',['group_id'=>Crypt::encrypt($group->id),'teacher_id'=>Crypt::encrypt($group->teacher_id)]) }}"
                       class="action-btn-xs evaluate" style="width:auto;padding:0 14px;font-size: 15.2px;gap:5px;display:inline-flex;">
        ...
        @empty
            <div class="marks-table-wrapper" style="padding: 40px; text-align: center; color: var(--light-text);">
                <i class="fa fa-bar-chart" style="font-size: 43.2px;opacity:.2;display:block;margin-bottom:10px;"></i>
