...
<div class="ajax-content" style="font-family:inherit; color:var(--text-primary);">
    ...
    <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-bottom:18px;">
        <div style="background:linear-gradient(135deg,#065f46,#10b981); color:#fff; border-radius:12px; padding:14px; text-align:center;">
            <div style="font-size: 15.2px; opacity:.85;">Total Received</div>
            <div style="font-size: 23.2px; font-weight:800;">{{ number_format($totalReceived,2) }} <small>ILS</small></div>
        </div>
        <div style="background:linear-gradient(135deg,#1e3a8a,#3b82f6); color:#fff; border-radius:12px; padding:14px; text-align:center;">
            <div style="font-size: 15.2px; opacity:.85;">Total Lectures</div>
            <div style="font-size: 23.2px; font-weight:800;">{{ $totalLectures }}</div>
        </div>
        <div style="background:linear-gradient(135deg,#7c3aed,#a78bfa); color:#fff; border-radius:12px; padding:14px; text-align:center;">
            <div style="font-size: 15.2px; opacity:.85;">Forms Count</div>
            <div style="font-size: 23.2px; font-weight:800;">{{ $forms->count() }}</div>
        </div>
    </div>
        ...
        <div style="border:1px solid var(--border-color); border-radius:12px; margin-bottom:14px; overflow:hidden;">
            ...
            <div style="display:flex; flex-wrap:wrap; justify-content:space-between; align-items:center; gap:10px; padding:12px 16px; background:var(--surface-2); border-bottom:1px solid var(--border-color);">
                <div style="font-weight:800; color:var(--text-primary);">
                    ...
                    <span style="background:{{ $st[1] }}1a; color:{{ $st[1] }}; font-size: 14.2px; font-weight:700; padding:3px 10px; border-radius:999px; margin-inline-start:8px;">{{ $st[0] }}</span>
                </div>
                <div style="font-size: 21.2px; font-weight:800; color:#16a34a;">{{ number_format($form->net_amount,2) }} <small style="color:var(--text-secondary);">ILS</small></div>
            </div>

            {{-- Figures --}}
            <div style="display:grid; grid-template-columns:repeat(5,1fr); gap:8px; padding:12px 16px; text-align:center; font-size: 15.2px;">
            ...
            @if($form->details->count())
                <details style="border-top:1px dashed var(--border-color);">
                    <summary style="cursor:pointer; padding:10px 16px; font-size: 16.2px; font-weight:700; color:var(--d-accent);">
                        <i class="fa fa-list-ul"></i> Calculated lecture details ({{ $form->details->count() }})
                    </summary>
                    <div style="padding:0 16px 14px;">
                        <table style="width:100%; border-collapse:collapse; font-size: 15.7px;">
                            <thead>
